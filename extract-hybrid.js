...
function isEnterprise(handle, text = '') {
  const h = normalizeHandle(handle);
  const hay = `${h} ${String(text).toLowerCase()}`;
  return enterprisePatterns.some((p) => hay.includes(p));
}

function isSameIndustryProvider(handle, text = '') {
  const h = normalizeHandle(handle);
  const hay = `${h} ${String(text).toLowerCase()}`;
  const compact = hay.replace(/\s+/g, '');
  return sameIndustryProviderPatterns.some((p) => {
...
function prospectFromResult(result) {
  ...
  if (isEnterprise(handle, `${result.title || ''} ${handle}`)) return null;
  if (isSameIndustryProvider(handle, text)) return null;
  const recruitSignal = /(採用|求人|新卒|中途|recruit|career|saiyo|jinji|staff|社員|募集)/i.test(text);
  if (!recruitSignal) return null;
  const localFit = localSignals.filter((w) => text.toLowerCase().includes(w)).length;
...
async function discoverInstagramFromCompanySites(keyword, industry, limit) {
  const sectorWords = industry && industry !== 'all' ? industry.replace('採用', '') : '介護 OR 歯科 OR 美容室 OR 建設 OR 保育 OR 物流 OR 店舗';
  const queries = [
  ...
  for (const item of siteItems) {
    ...
    try {
      ...
      for (const r of matches) {
        ...
        const p = prospectFromResult({ handle, title: item.title, snippet: resultText, engine: '公式サイト→Instagram抽出' });
        if (!p) continue;
...
async function discover(keyword, industry, limit) {
  if (isSameIndustryProvider('', keyword)) return [];
  const qBase = [keyword || '採用', industry && industry !== 'all' ? industry : '', '中小企業 OR 店舗 OR クリニック OR 介護 OR 建設 OR 美容', 'site:instagram.com', '(採用 OR 求人 OR recruit OR career OR saiyo)'].filter(Boolean).join(' ');
  const queries = [
    qBase,
  ...
  if (byHandle.size < limit) {
    const siteProspects = await discoverInstagramFromCompanySites(keyword, industry, limit - byHandle.size);
...
async function discoverScheduled(industry, limit) {
  ...
  for (const job of jobs) {
    if (byHandle.size >= limit) break;
    const rows = await discover(job.keyword, targetIndustry === 'all' ? 'all' : job.industry, Math.min(8, Math.max(3, limit - byHandle.size)));
    for (const p of rows) {
      const key = normalizeHandle(p.account);
...
module.exports = async function handler(req, res) {
  ...
  try {
    const prospects = mode === 'scheduled' ? await discoverScheduled(industry, limit) : await discover(keyword, industry, limit);
    return json(res, 200, {
      ok: true,
