...
function defaultDb() {
  return {
    ...
    scheduler: {
      ...
      dailySalesLimit: 32,
      monthlySalesLimit: 704,
      weeklyBuzzLimit: 74,
      monthlyBuzzLimit: 296,
      quotaPolicy: '月1000件以内。営業候補32件/日×22営業日=704件、トレンド投稿74件/週×4週=296件。',
...
function readDb() {
  try {
    ...
    const scheduler = { ...defaults.scheduler, ...(db.scheduler || {}) };
    if (!db.scheduler?.quotaPolicy || Number(scheduler.limit) === 20) scheduler.limit = defaults.scheduler.limit;
    scheduler.dailySalesLimit = defaults.scheduler.dailySalesLimit;
    scheduler.monthlyQuota = defaults.scheduler.monthlyQuota;
    scheduler.businessDaysPerMonth = defaults.scheduler.businessDaysPerMonth;
    scheduler.monthlySalesLimit = defaults.scheduler.monthlySalesLimit;
    scheduler.weeklyBuzzLimit = defaults.scheduler.weeklyBuzzLimit;
    scheduler.monthlyBuzzLimit = defaults.scheduler.monthlyBuzzLimit;
    scheduler.quotaPolicy = defaults.scheduler.quotaPolicy;
    return { ...defaults, ...db, scheduler };
  } catch (e) {
    return defaultDb();
  }
}
