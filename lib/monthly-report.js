...
function defaultInsightsUpstream() {
  ...
  return base ? `${base.replace(/\/$/, '')}/api/instagram-insights` : '';
}
async function fetchInsights(payload) {
  ...
  if (!upstream) {
    if (allowDemo) return { insights: demoInsights(payload), source: 'demo_db_only' };
    const err = new Error('instagram_insights_upstream_not_configured');
    err.code = 'missing_instagram_upstream';
    throw err;
  }
